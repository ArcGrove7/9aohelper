...
//   然後是 14 個品質等級的欄位——格子裡的數字是**打出該品質所需的數量**。
//   （例：鐵放 1 個就能到「次等」以上；泥土要 2 個才勉強到「屎一般」。）
//
// 這兩件事站上 materials.html 都沒有：完整的 14 級品質階梯，以及品質門檻。
...
function main() {
  ...
  const md = ['# 素材係數與品質門檻（社群表）', '',
    ...
    '品質欄位的數字是**打出該品質所需的數量**——例如鐵放 1 個就能到「次等」以上，',
    '泥土要 2 個才勉強到「屎一般」。空白表示這張表沒有該品質的資料。', ''];
  ...
  for (const cat of json.categories) {
    ...
    md.push('| 素材 | 攻 | 防 | 幸 | 重 | 耐 | 特效 |' + (anyQ ? ' 品質門檻 |' : ''));
    md.push('|---|---|---|---|---|---|---|' + (anyQ ? '---|' : ''));
    for (const m of rows) {
