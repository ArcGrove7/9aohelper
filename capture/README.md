...
- **社群表與站上的數值是同一套，只差固定係數**。拿泥土、石頭、兔皮三種交叉比對：
  ...
  補完 382 種，黑暗山洞的蝙蝠翅膀、夜明砂這些才用得上。站上有的仍以站上為準。
- **社群表的「類別」是產地不是材質標籤**（大草原、蝙蝠洞、鷹洞…共 31 個），
  所以不併進 tags——泥土類／木頭類的判定仍以站上的標籤為準。
- **品質有 14 級**：屎一般 → 垃圾般 → 破爛 → 劣質 → 次等 → 普通 → 上等 → 高級 →
  精良 → 頂級 → 完美 → 史詩 → 神話 → 傳說。社群表的品質欄位看起來是
  「打出該品質所需的該素材數量」（鐵 1 個就到次等，泥土要 2 個才到屎一般），
  但這是從數字推的，還沒實測驗證過。
